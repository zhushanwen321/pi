...
describe("OpenAI Codex OAuth", () => {
	...
	it("does not treat unrelated OpenAI Codex device auth 403 responses as pending", async () => {
		vi.stubGlobal(
			"fetch",
			vi.fn(async (input: unknown): Promise<Response> => {
				...
				if (url === "https://auth.openai.com/api/accounts/deviceauth/token") {
					return jsonResponse({ error: "access_denied", error_description: "denied" }, 403);
				}
				throw new Error(`Unexpected fetch URL: ${url}`);
			}),
		);
		...
		await expect(
			...
			'OpenAI Codex device auth failed with status 403: {"error":"access_denied","error_description":"denied"}',
		);
	});
