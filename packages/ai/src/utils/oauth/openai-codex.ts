...
async function readTokenResponse(response: Response, operation: TokenOperation): Promise<OAuthToken> {
	...
	if (typeof accessToken !== "string" || typeof refreshToken !== "string" || typeof expiresIn !== "number") {
		throw new Error(`OpenAI Codex token ${operation} response missing fields: ${JSON.stringify(raw)}`);
	}
...
function responseDetails(responseBody: string): string {
	return responseBody ? `: ${responseBody}` : "";
}

async function readResponseDetails(response: Response): Promise<string> {
	return responseDetails(await response.text().catch(() => ""));
}
...
function parseDeviceAuthPollFailure(
	status: number,
	responseBody: string,
): { status: "pending" } | { status: "slow_down" } | { status: "failed"; message: string } {
	const json = parseObjectJson(responseBody, "Invalid OpenAI Codex device auth error response");
	...
	return {
		status: "failed",
		message: `OpenAI Codex device auth failed with status ${status}${responseDetails(responseBody)}`,
	};
}
...
async function pollOpenAICodexDeviceAuth(device: DeviceAuthInfo, signal?: AbortSignal): Promise<DeviceTokenSuccess> {
	return pollOAuthDeviceCodeFlow<DeviceTokenSuccess>({
		...
		poll: async () => {
			...
			if (response.ok) {
				return { status: "complete", value: parseDeviceTokenSuccess(responseBody) };
			}

			return parseDeviceAuthPollFailure(response.status, responseBody);
		},
	});
}
